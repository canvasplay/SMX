...
function(Track, TrackDataTypes){


	var TrackDataManager = function(document, playhead){

		//document && playhead params are required
		if(!document || !playhead) return;
		...
		this.attrControllers = TrackDataTypes;
	...
	TrackDataManager.prototype.initializeDocument = function(){
		...
		for (var n=0; n<nodes.length; n++){
			...
			if (is_tracking){
				...
				if (node.hasParent()) track_attrs.parent = node.getParent().id;

				//create a new Track with catched attributes
				var track = new Track(track_attrs);
	...
	TrackManager.prototype.get = function(id, key, format){
		...
		//key attr controller? and has get method?
		var attrController = this.attrControllers[key];
		if(attrController && attrController.get)
			value = attrController.get(track,this,format);
		else
		//use direct get in track
			value = track.get(key);
	...
	TrackManager.prototype.set = function(id, key, value){
		...
		if(handler && handler.set)	return handler.set(value, track, this);
	...
	TrackManager.prototype.onCollectionChange = function(track){
		...
		if(track.changed){
			...
			for (var i=0; i< keys.length; i++){
				...
				if(handler.onchange){
					handler.onchange(track, this, previous[keys[i]],previous_value);
				}				
	...
	*/
	...
	TrackManager.prototype.toJSON = function (options){
		...
		var defaults = {
			...
			'onlychanged': false	// if true will only output changed attributes
		}
	...
	TrackManager.prototype.setJSON = function (myJSON){
		...
		for (var i=len-1; i>0;i--){
			...
			try{
				...
				if (item.access) 	track.set('access', item.access, true);
				if (item.views) 	track.set('views', item.views, true);
				if (item.progress) 	track.set('progress', item.progress, true);
				if (item.score) 	track.set('score', item.score, true);
				if (item.status) 	track.set('status', item.status, true);
